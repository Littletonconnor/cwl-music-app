...
export function useKeyboardNavigation() {
	...
	const panelRefs = React.useRef<
		Record<Panel, React.RefObject<HTMLElement> | null>
	>({
		sidebar: null,
		songlist: null,
	});

	const registerPanelRef = React.useCallback(
	...
	const handleKeyNavigation = React.useCallback(
		(panel: Panel, e: React.KeyboardEvent) => {
			...
			switch (e.key) {
				...
				case "k":
					e.preventDefault();
					// TODO: this might need to be (currentIndex - 1 + items.length)
				...
				case "l":
					if (panel === "sidebar") {
						...
						const songlistFirstItem =
							panelRefs.current.songlist?.current?.querySelector(
								'[tabindex="0"]',
							) as HTMLElement | null;
				...
				default:
					throw new Error(`Invalid key pressed: ${e.key}`);
			}
		},
		[],
	);
